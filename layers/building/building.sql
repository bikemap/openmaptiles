...
CREATE OR REPLACE VIEW osm_all_buildings AS (
         ...
         SELECT member AS osm_id,geometry,
                  COALESCE(nullif(omt_as_numeric(height),-1),nullif(omt_as_numeric(buildingheight),-1)) as height,
                  COALESCE(nullif(omt_as_numeric(min_height),-1),nullif(omt_as_numeric(buildingmin_height),-1)) as min_height,
                  COALESCE(nullif(omt_as_numeric(levels),-1),nullif(omt_as_numeric(buildinglevels),-1)) as levels,
                  COALESCE(nullif(omt_as_numeric(min_level),-1),nullif(omt_as_numeric(buildingmin_level),-1)) as min_level,
         ...
         SELECT obp.osm_id,obp.geometry,
                  COALESCE(nullif(omt_as_numeric(obp.height),-1),nullif(omt_as_numeric(obp.buildingheight),-1)) as height,
                  COALESCE(nullif(omt_as_numeric(obp.min_height),-1),nullif(omt_as_numeric(obp.buildingmin_height),-1)) as min_height,
                  COALESCE(nullif(omt_as_numeric(obp.levels),-1),nullif(omt_as_numeric(obp.buildinglevels),-1)) as levels,
                  COALESCE(nullif(omt_as_numeric(obp.min_level),-1),nullif(omt_as_numeric(obp.buildingmin_level),-1)) as min_level,
