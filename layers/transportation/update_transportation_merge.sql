           ...
           DENSE_RANK() OVER (
               ORDER BY highway, network, construction, is_bridge, is_tunnel, is_ford, expressway, bicycle,
                        foot, horse, mtb_scale, sac_scale, access, toll, layer, cycleway_value(
                            cycleway, cycleway_both, cycleway_left, cycleway_right, cycleway_street
                        )
           ) as cluster_group
    FROM (
...
UPDATE osm_transportation_merge_linestring_gen_z11 SET bm_weight_tracked = q.bm_weight_tracked
FROM (
    SELECT s.id, ROUND(SUM(NULLIF(bm_weight_tracked, '')::NUMERIC))::int AS bm_weight_tracked
...
BEGIN
    ...
    ON CONFLICT (id) DO UPDATE SET osm_id = excluded.osm_id, highway = excluded.highway, network = excluded.network,
                                   ...
                                   cycleway = excluded.cycleway, bm_weight_tracked = excluded.bm_weight_tracked;
...
-- Update Route-Processing Aggregations
-- Execute after indexes have been created on osm_transportation_merge_linestring_gen_z11 to improve performance
UPDATE osm_transportation_merge_linestring_gen_z8 SET  bm_weight_tracked = q.bm_weight_tracked
FROM (
    SELECT s.id, SUM(bm_weight_tracked)::int AS bm_weight_tracked
...
BEGIN
    ...
    SELECT osm_highway_linestring_gen_z11.osm_id, NULL::INTEGER AS id, NULL::BIGINT[] AS source_ids,
           ...
           visible_layer(geometry, layer, 11) AS layer, cycleway_value(
               cycleway, cycleway_both, cycleway_left,
               cycleway_right, cycleway_street
           ) AS cycleway, z_order
    ...
    SELECT NULL::BIGINT AS osm_id, m.id,
           ...
           visible_layer(m.geometry, m.layer, 11) AS layer, m.cycleway, m.z_order
    ...
    WITH inserted_linestrings AS (
        ...
        INSERT INTO osm_transportation_merge_linestring_gen_z11(geometry, new_source_ids, old_source_ids, highway,
                                                                ...
                                                                sac_scale, access, toll, layer, cycleway)
        SELECT (ST_Dump(ST_LineMerge(ST_Union(geometry)))).geom AS geometry,
               -- We use St_Union instead of St_Collect to ensure no overlapping points exist within the geometries to
               -- merge. https://postgis.net/docs/ST_Union.html
               -- ST_LineMerge only merges across singular intersections and groups its output into a MultiLineString if
    ...
    UPDATE osm_transportation_merge_linestring_gen_z11 SET bm_weight_tracked = q.bm_weight_tracked
    FROM (
        SELECT updated_relations.id, ROUND(SUM(NULLIF(bm_weight_tracked, '')::NUMERIC))::int AS bm_weight_tracked
...
BEGIN
    ...
    UPDATE osm_transportation_merge_linestring_gen_z8 SET bm_weight_tracked = q.bm_weight_tracked
    FROM (
        SELECT updated_relations.id, SUM(bm_weight_tracked)::int AS bm_weight_tracked
        FROM updated_relations
        JOIN osm_transportation_merge_linestring_gen_z9 ON (
