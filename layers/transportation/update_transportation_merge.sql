...
CREATE INDEX IF NOT EXISTS osm_transportation_merge_linestring_gen_z11_update_partial_idx
    ON osm_transportation_merge_linestring_gen_z11 (highway, construction)
...
CREATE INDEX IF NOT EXISTS osm_transportation_merge_linestring_gen_z9_update_partial_idx
    ON osm_transportation_merge_linestring_gen_z9 (highway, construction, ST_IsValid(geometry), access)
    WHERE (
...
CREATE INDEX IF NOT EXISTS osm_transportation_merge_linestring_gen_z8_update_partial_idx
    ON osm_transportation_merge_linestring_gen_z8 (ST_Length(geometry))
...
BEGIN
    ...
    SELECT osm_id, NULL::INTEGER AS id, geometry, highway, network, construction,
           visible_brunnel(geometry, is_bridge, 11) AS is_bridge, visible_brunnel(geometry, is_tunnel, 11) AS is_tunnel,
           visible_brunnel(geometry, is_ford, 11) AS is_ford, expressway, bicycle, foot, horse, mtb_scale, sac_scale,
    ...
    SELECT unnest(source_ids) AS osm_id, id,
           geometry, highway, network, construction, visible_brunnel(geometry, is_bridge, 11) AS is_bridge,
           visible_brunnel(geometry, is_tunnel, 11) AS is_tunnel, visible_brunnel(geometry, is_ford, 11) AS is_ford,
...
BEGIN
    ...
    SELECT id AS source_id, NULL::int AS id, geometry, highway, network, construction,
           visible_brunnel(geometry, is_bridge, 9) AS is_bridge, visible_brunnel(geometry, is_tunnel, 9) AS is_tunnel,
    ...
    SELECT unnest(source_ids) AS source_id, id, geometry, highway, network, construction,
           visible_brunnel(geometry, is_bridge, 9) AS is_bridge, visible_brunnel(geometry, is_tunnel, 9) AS is_tunnel,
           visible_brunnel(geometry, is_ford, 9), expressway, z_order
    FROM osm_transportation_merge_linestring_gen_z8
    WHERE EXISTS(
        SELECT NULL FROM linestrings_to_merge
        WHERE ST_Intersects(
