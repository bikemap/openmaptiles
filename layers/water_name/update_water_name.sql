...
CREATE INDEX IF NOT EXISTS osm_water_polygon_partial_idx ON osm_water_polygon (name, ST_IsValid(geometry))
    WHERE name <> '' AND ST_IsValid(geometry);
CREATE INDEX IF NOT EXISTS osm_water_polygon_osm_id_idx ON osm_water_polygon (osm_id);
...
CREATE INDEX IF NOT EXISTS water_name_osm_ids_osm_id_idx ON water_name.osm_ids (osm_id);
...
BEGIN
    ...
    ANALYZE VERBOSE water_name.osm_ids;
